#!/usr/bin/env python3
"""Trial runner for the autoresearch loop — Claude Code CLI edition.

Identical to run_trial.py but uses judge_claude_code instead of judge,
so no ANTHROPIC_API_KEY is required. The `claude` CLI must be installed
and authenticated (https://claude.ai/code).

Prints the mean score to stdout as:
...
Usage:
    python experiments/run_trial_claude_code.py
    python experiments/run_trial_claude_code.py --strategy dark --threshold 50
"""
...
import judge_claude_code as judge                   # noqa: E402
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Run one autoresearch trial (Claude Code CLI judge)")
...
def main() -> None:
    ...
    print(file=sys.stderr)
    print("Calling LLM judge (Claude Code CLI)...", file=sys.stderr)
    ...
    # Only stdout line — parsed by loop.py / loop_claude_code.py
